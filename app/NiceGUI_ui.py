...
def transcribe_with_status():
    global uploaded_file, status_label, result_box, progress
...
def transcribe_nicegui_ui():
    global uploaded_file, status_label, result_box, progress, file_name_label
    ...
    with ui.column().classes('items-center').style('gap: 20px; max-width: 700px; margin: auto'):
        ...
        ui.label('下のボタンをクリックしてファイルを選んでください').style('color: gray')
